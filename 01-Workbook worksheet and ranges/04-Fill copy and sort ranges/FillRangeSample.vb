...
Namespace EPPlusSamples.WorkbookWorksheetAndRanges
    Public Module FillRangeSample
        Public Sub Run()
            Using p = New ExcelPackage()
                FillNumber(p)
                FillDate(p)
                FillList(p)
                p.SaveAs(FileUtil.GetCleanFileInfo("1.04-FillRangeSamples.xlsx"))
            End Using
        End Sub
        ''' <summary>
        ...
        Private Sub FillNumber(ByVal p As ExcelPackage)
            Dim ws = p.Workbook.Worksheets.Add("FillNumber Samples")
            ...
            ws.Cells("E1:AA1").FillNumber(Sub(x)
        ...
        ''' </summary>
        ''' <paramname="p">The package</param>
        Private Sub FillDate(ByVal p As ExcelPackage)
            Dim ws = p.Workbook.Worksheets.Add("FillDateTime Samples")
            ...
            ws.Cells("C2:D60").FillDateTime(Sub(x)
                                                ...
                                                x.SetExcludedDates(New DateTime(2010, 12, 31), New DateTime(2012, 12, 31), New DateTime(2013, 12, 31), New DateTime(2014, 12, 31), New DateTime(2015, 12, 31), New DateTime(2015, 12, 31), New DateTime(2018, 12, 31), New DateTime(2019, 12, 31), New DateTime(2020, 12, 31), New DateTime(2021, 12, 31), New DateTime(2024, 12, 31), New DateTime(2025, 12, 31), New DateTime(2026, 12, 31), New DateTime(2027, 12, 31), New DateTime(2029, 12, 31))                                          'These dates are also excluded. The day before is used instead.
                                            End Sub)
            ...
            ws.Cells("H1:AA4").FillDateTime(Sub(x)
                                                x.Direction = eFillDirection.Row
        ...
        ''' </summary>
        ''' <paramname="p">The package</param>
        Private Sub FillList(ByVal p As ExcelPackage)
            Dim ws = p.Workbook.Worksheets.Add("Fill List Samples")

            Dim list = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"}
            ...
            ws.Cells("B2:B20").FillList(list, Sub(x) x.StartIndex = 6)

            ws.SetValue("E1", "FillList-Per row")
            'Fill per row
            ws.Cells("F1:AA1").FillList(list, Sub(x) x.Direction = eFillDirection.Row)
            ...
            Dim primes = New List(Of Integer) From {
                2,
                5,
                7,
                11,
            ...
            ws.Cells(2, 3, primes.Count + 1, 3).FillList(primes, Sub(x)
                                                                     x.NumberFormat = "#,##0"
                                                                     x.StartPosition = eFillStartPosition.BottomRight
                                                                 End Sub)
