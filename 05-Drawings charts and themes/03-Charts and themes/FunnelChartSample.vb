...
Namespace EPPlusSamples.DrawingsChartsAndThemes
    Public Class FunnelChartSample
        Public Shared Sub Add(ByVal package As ExcelPackage)
            Dim ws = LoadFunnelChartData(package)

            Dim funnelChart = ws.Drawings.AddFunnelChart("FunnelChart")
            funnelChart.Title.Text = "Sales process"
            funnelChart.SetPosition(1, 0, 6, 0)
            funnelChart.SetSize(800, 400)
            Dim fSerie = funnelChart.Series.Add(ws.Cells(2, 2, 7, 2), ws.Cells(2, 1, 7, 1))
            fSerie.DataLabel.Add(False, True)
            funnelChart.StyleManager.SetChartStyle(Drawing.Chart.Style.ePresetChartStyle.FunnelChartStyle9)
        End Sub

        Private Shared Function LoadFunnelChartData(ByVal package As ExcelPackage) As ExcelWorksheet
            Dim ws = package.Workbook.Worksheets.Add("FunnelChart")
